...
export default function CheckoutFlow({ event, tickets, ticketTypes, user }: { event: Event, tickets: Ticket[]; ticketTypes: EventTicketType[]; user: User }) {
  ...
  type TCheckoutSchema = z.infer<ReturnType<typeof createCheckoutSchema>>;
  ...
  const steps = [
    {
      name: t('footer.ticket'),
      fields: ['tickets']
    },
    {
      name: t('footer.recipient'),
      fields: ['tickets']
    },
    {
      name: t('footer.payment'),
      fields: ['tickets']
    }
  ]
  ...
  return (
    <>
      ...
      <div className='relative h-full min-h-0 flex flex-col'>
        ...
        <main className='grid grid-cols-1 lg:grid-cols-[29fr_23fr] w-full h-full gap-8 flex-1 min-h-0'>
          ...
          {currentStep === 0 && (
            <motion.div
              ...
              layout={false}
              className='flex flex-col gap-20 min-h-0 h-full '
            >
              <ul className="flex flex-col gap-8 overflow-y-auto h-full min-h-0">
