...
export default function EmailNotifications({ userPreferences }: { userPreferences: UserPreference }) {
    ...
    async function updatePreferences(e: React.FormEvent<HTMLFormElement>) {
        ...
        const newEventsPreferredCategories = formData.get('newEventsPreferredCategories') === 'on'
        const newEventsFollowedOrganizer = formData.get('newEventsFollowedOrganizer') === 'on'
        ...
        const response = await UpdateUserPreferences(session?.user.accessToken ?? '', body)
    ...
    return (
        <>
            ...
            <form onChange={updatePreferences} className='flex flex-col gap-6'>
                <span className='font-medium text-[1.8rem] mb-4 leading-[25px] text-deep-100'>{t('email.title')}</span>
