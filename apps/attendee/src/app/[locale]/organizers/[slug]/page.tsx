...
export default async function OrganizerProfile({
    ...
    const organisationRequest = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/organisations/${slug}`, {
        method: "GET",
        headers: {
            ...
            <main className='w-full gap-8 flex flex-col lg:grid lg:grid-cols-[29fr_23fr] lg:min-h-0 overflow-y-auto h-full'>
                <div className="flex flex-col gap-8 overflow-y-auto min-h-0">
                    ...
                    <div className="w-full max-h-[298px] overflow-hidden rounded-[10px] flex-shrink-0">
                        {organisation.profileImageUrl ? <Image
                            src={organisation.profileImageUrl}
                            width={580}
                            height={298}
                            alt={organisation.organisationName}
                            className="w-full"
                        /> : <div className='w-full h-[298px] font-primary text-white bg-black text-9xl flex justify-center items-center'> {organisation.organisationName.slice()[0]?.toUpperCase()}</div>}
                    </div>
                    <OrganizerActions events={events} organisation={organisation} user={session?.user as User} />
                    <Separator />
                    <div className='flex flex-col gap-4'>
                        <span className='font-semibold text-[1.6rem] leading-8 text-deep-100'>{t('profile.about')}</span>
                        <p className='text-[1.5rem] leading-12 text-neutral-700'>{organisation.organisationDescription}</p>
                    </div>
                    <Separator />
                    <div>
                        <span className='font-semibold text-[1.6rem] leading-8 text-deep-100'>{t('profile.contact')}</span>
                    </div>
