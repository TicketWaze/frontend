...
export default function TicketViewer({ tickets, event }: { tickets: Ticket[]; event: Event }) {
    ...
    const isLabColor = (colorValue: string): boolean => {
        return colorValue.includes('lab(') || colorValue.includes('lch(') || colorValue.includes('oklab(') || colorValue.includes('oklch(');
    };
    ...
    const handleCanvasElements = (element: HTMLElement, originalElement: HTMLElement) => {
    ...
    const applyInlineStyles = (element: HTMLElement, originalElement: HTMLElement) => {
        const computed = window.getComputedStyle(element);
        
        // Fix background color
        const bgColor = computed.backgroundColor;
        if (isLabColor(bgColor)) {
            element.style.backgroundColor = bgColor.includes('neutral') ? '#f5f5f5' : '#ffffff';
        ...
        if (isLabColor(textColor)) {
            element.style.color = textColor.includes('neutral') ? '#666666' : 
                                 textColor.includes('primary') ? '#E45B00' : 
                                 textColor.includes('deep') ? '#0D0D0D' : '#000000';
        } else if (textColor) {
        ...
        if (isLabColor(borderColor)) {
            element.style.borderColor = borderColor.includes('primary') ? '#E45B00' : 
                                       borderColor.includes('neutral') ? '#e5e5e5' : '#cccccc';
    ...
    const downloadImage = async () => {
        ...
        try {
            ...
            try {
                ...
                alert('Failed to generate ticket image. Please try again or contact support.');
            }
        }
    };
    ...
    const downloadPDF = async () => {
        ...
        try {
            ...
            const yOffset = pdfHeight < pdf.internal.pageSize.getHeight() ? 
                           (pdf.internal.pageSize.getHeight() - pdfHeight) / 2 : 0;
    ...
    return (
        <>
            ...
            <div className="absolute -left-[9999px] opacity-0 pointer-events-none">
                <div ref={ticketRef} className="bg-white shadow-lg p-6 rounded-xl w-full text-center flex flex-col gap-8 items-center">
                <div
                    className={
                ...
                >
                    <div className={'flex items-center justify-between gap-4 w-full'}>
                        <span className='text-neutral-600'>1x {Capitalize(tickets[currentIndex].ticketType)}</span>
                        {tickets[currentIndex].ticketPrice < 1 ? <span className='text-deep-100 font-medium'>{t('free')}</span> : null}
                    </div>
                    <div className='flex flex-col gap-4 w-full'>
                        <div className='h-[2px] w-full rounded-[10px] bg-neutral-200'></div>
                        <div className={'flex items-center justify-between gap-4 w-full'}>
                            <span className='text-neutral-600'>{t('ticketId')}</span>
                            <span className='text-primary-500 font-medium'>{tickets[currentIndex].ticketName}</span>
                        </div>
                        <div className='h-[2px] w-full rounded-[10px] bg-neutral-200'></div>
                        <div className={'flex items-center justify-between gap-4 w-full'}>
            ...
            <div className="border border-neutral-100 rounded-[100px] py-4 px-[1.5rem] flex justify-between">
                <div className="flex items-center gap-[18px]">
                    <button onClick={goToPrevious} disabled={mockTickets.length <= 1} className="w-[35px] cursor-pointer h-[35px] disabled:cursor-not-allowed rounded-full bg-neutral-100 flex items-center justify-center">
                        <ArrowLeft2 variant="Bulk" size={20} color="#0D0D0D" />
                    </button>
                    <span className="text-[2.2rem] leading-12 text-neutral-600"><span className="text-primary-500">{currentIndex + 1}</span>/{mockTickets.length}</span>
                    <button onClick={goToNext} disabled={mockTickets.length <= 1} className="w-[35px] cursor-pointer h-[35px] disabled:cursor-not-allowed rounded-full bg-neutral-100 flex items-center justify-center">
                        <ArrowRight2 variant="Bulk" size={20} color="#0D0D0D" />
                    </button>
                </div>
                <button onClick={downloadPDF} className="border-2 cursor-pointer border-primary-500 px-12 py-[7.5px] bg-[#FFEFE2] rounded-[100px] flex gap-4 items-center justify-center">
                    <DocumentDownload variant="Bulk" size={20} color="#E45B00" className="hidden lg:block" />
                    <span className="text-[1.5rem] text-primary-500">{t('download')}</span>
                </button>
            </div>
        </>
    );
