'use client'
import {  DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@workspace/ui/components/dialog"
...
export default function EditMemberDialogContent({ userId, defaultRole, user }: { userId: string; defaultRole: string; user : User }) {
    ...
    async function submitHandler(data: TAddMemberSchema) {
        setIsLoading(true)
        const result = await EditMemberAction(organisation?.organisationId ?? '', userId, user.accessToken, data.role)
