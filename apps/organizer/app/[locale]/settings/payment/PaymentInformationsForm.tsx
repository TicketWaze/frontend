...
export default function PaymentInformationsForm() {
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<TPaymentDetailsSchema>({
    ...
    async function submitHandler(data: TPaymentDetailsSchema) {
        const result = await UpdateOrganisationPaymentInformation(organisation?.organisationId ?? '', data.bankName, data.bankAccountName, data.bankAccountNumber, session?.user.accessToken ?? '')        
    ...
    return (
        <div
            className={'flex flex-col overflow-y-scroll gap-[40px] w-full h-full  lg:w-[530px] mx-auto'}
        >
            <form id='payment-form' onSubmit={handleSubmit(submitHandler)} className={'flex h-[70vh] flex-col gap-8'}>
                <span className={'pb-4 font-medium text-[1.8rem] leading-[25px] text-deep-100'}>
                    {t('subtitle')}
                </span>

                <div
                    className={'border border-neutral-100 rounded-[10px] p-[15px] flex flex-col gap-[15px]'}
                >
                    <span className={'font-semibold text-[16px] leading-[22px] text-deep-100'}>
                        {t('bank')}
                    </span>
                    <Input {...register('bankName')} type='text' disabled={isSubmitting} error={errors.bankName?.message}>{t('bank_name')}</Input>
                    <Input {...register('bankAccountName')} type='text' disabled={isSubmitting} error={errors.bankAccountName?.message}>{t('bank_account_name')}</Input>
                    <Input {...register('bankAccountNumber')} type='text' disabled={isSubmitting} error={errors.bankAccountNumber?.message}>{t('bank_account_number')}</Input>
                </div>
            </form>
        </div>
    )
