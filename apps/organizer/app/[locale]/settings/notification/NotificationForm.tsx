...
export default function NotificationForm({ notificationPreferences }: { notificationPreferences: NotificationPreference }) {
    ...
    async function changeHandler(e: React.FormEvent<HTMLFormElement>) {
        ...
        const emailPlatformAnnouncements = formData.get('emailPlatformAnnouncements') === 'on'
        ...
        const result =  await UpdateOrganisationNotificationPreferences(organisation?.organisationId ?? '', body, session?.user.accessToken ?? '')
    ...
    return (
        <div className={'flex flex-col overflow-y-scroll gap-[40px] w-full h-[70dvh] lg:w-[530px] mx-auto'}>
            <form onChange={changeHandler} className={'flex flex-col gap-8'}>
                <span className={'font-medium text-[1.8rem] pb-4 leading-[25px] text-deep-100'}>
                    {t('email')}
                </span>
                ...
                <div className={'flex items-center justify-between'}>
                    ...
                    <label className="relative inline-block h-[30px] w-[50px] cursor-pointer rounded-full bg-neutral-600 transition [-webkit-tap-highlight-color:_transparent] has-[:checked]:bg-primary-500">
                        <input
                            ...
                            defaultChecked={notificationPreferences.emailPlatformAnnouncements}
