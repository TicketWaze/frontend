...
export default function ChangePasswordForm() {
    ...
    async function submitHandler(data: TChangePasswordSchema) {
        try {
            const response = await post('/users/me/change-password', session?.user.accessToken ?? '', data )
    ...
    return (
        <form onSubmit={handleSubmit(submitHandler)} className={'flex flex-col gap-8'}>
            ...
            <PasswordInput {...register('currentPassword')} name='currentPassword' error={errors.currentPassword?.message} isLoading={isSubmitting} >{t('placeholders.password')}</PasswordInput>
            <PasswordInput {...register('password')} name='password' error={errors.password?.message} isLoading={isSubmitting}>{t('placeholders.new')}</PasswordInput>
            <PasswordInput {...register('password_confirmation')} name='password_confirmation' error={errors.password_confirmation?.message} isLoading={isSubmitting}>{t('placeholders.confirm')}</PasswordInput>
