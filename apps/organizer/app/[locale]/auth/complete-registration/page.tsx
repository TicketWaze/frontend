...
export default function CompleteRegistrationPage() {
  ...
  const { register, handleSubmit, setValue, formState: { errors, isSubmitting } } = useForm<TCompleteRegistrationSchema>({
        ...
        <div className='flex-1 flex lg:justify-center flex-col w-full pt-[4.5rem]'>
          <div className='flex flex-col gap-16 items-center'>
            <div className='flex flex-col gap-8 items-center'>
              <h3 className='font-medium font-primary text-center text-[3.2rem] leading-[3.5rem] text-black'>{t('title')}
              </h3>
            ...
            <div className=' w-full flex flex-col gap-6'>
              ...
              <div className={'flex gap-[1.5rem]'}>
                <Input {...register('state')} type='text' error={errors.state?.message}>{t('placeholders.state')}</Input>
                <Input {...register('city')} type='text' error={errors.city?.message}>{t('placeholders.city')}</Input>
              </div>
              <div>
                <div
                  className={
              ...
              <Input {...register('phoneNumber')} type='text' error={errors.phoneNumber?.message}>{t('placeholders.phone')}</Input>
              <div>
                <Select onValueChange={(e) => setValue('gender', e)}>
                  ...
                  <SelectContent className={'bg-neutral-100 text-[1.4rem]'}>
                    <SelectGroup>
                      <SelectItem className={'text-[1.4rem] text-deep-100'} value={'male'}>
                        {t('placeholders.male')}
                      </SelectItem>
                      <SelectItem className={'text-[1.4rem] text-deep-100'} value={'female'}>
                        {t('placeholders.female')}
                      </SelectItem>
                      <SelectItem className={'text-[1.4rem] text-deep-100'} value={'others'}>
                        {t('placeholders.no')}
                      </SelectItem>
                    </SelectGroup>
                  </SelectContent>
                ...
                {errors.gender && (
                  <span className={'text-[1.2rem] px-8 py-2 text-failure'}>{errors.gender.message}</span>
                )}
              </div>
              <div>
