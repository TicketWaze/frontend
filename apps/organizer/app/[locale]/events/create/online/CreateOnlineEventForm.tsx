...
export default function CreateOnlineEventForm({ tags }: { tags: EventTag[] }) {
        ...
        eventDays: z.array(z.object(
            {
                ...
                endTime: z.string().min(1, t('errors.dateAndTime.endTime'))
            })),
    ...
    const processForm: SubmitHandler<TInpuFormDataSchema> = async data => {
        ...
        const result = await CreateGoogleMeetEvent(organisation?.organisationId ?? '', session?.user.accessToken ?? '', formData, encodeURIComponent(searchParams.get('code') ?? ''))
