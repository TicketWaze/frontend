'use client'
import { CreateDiscountCode, MarkDiscountCodeAsActive, MarkDiscountCodeAsInactive } from '@/actions/EventActions'
...
import { CloseCircle, Money3, MoreCircle, RefreshCircle, TickCircle, TicketDiscount } from 'iconsax-react'
...
export default function DiscountPageContent({ event }: { event: Event }) {
  ...
  async function MarkAsInactive(discountCodeId: string) {
    const result = await MarkDiscountCodeAsInactive(event.eventId, discountCodeId, session?.user.accessToken ?? '', pathname)
  ...
  async function MarkAsActive(discountCodeId: string) {
    const result = await MarkDiscountCodeAsActive(event.eventId, discountCodeId, session?.user.accessToken ?? '', pathname)
    if (result.status === 'success') {
      toast.success('success')
    }
    if (result.error) {
      ...
      <TopBar title={t('subtitle')}>
        <Drawer direction={'right'}>
          <DrawerTrigger className={'w-full'}>
            <div className='flex items-center gap-4 border-primary-500 text-primary-500 bg-primary-100 px-[3rem] py-[15px] border-2 rounded-[100px] text-center  font-medium text-[1.5rem] h-auto leading-[20px] cursor-pointer transition-all duration-400 justify-center'>
                    ...
                    ({
                      ...
                      return (
                        <TableRow key={discountCodeId}>
                          ...
                          >
                            <span className={`${isActive ? 'text-success' : 'text-failure'}`}>{code}</span>
                          </TableCell>
                          <TableCell
                            className={
                          ...
                          >
                            <span className={`${isActive ? 'text-success' : 'text-failure'}`}>{Capitalize(type)}</span>
                          </TableCell>
                          <TableCell className={'text-[1.5rem] leading-8 text-neutral-900'}>
                            <span className={`${isActive ? 'text-success' : 'text-failure'}`}>
                              {type === 'percentage' ? value + '%' : value}
                            </span>
                          </TableCell>
                          <TableCell className={'text-[1.5rem] leading-8 text-neutral-900'}>
                            <span className={`${isActive ? 'text-success' : 'text-failure'}`}>{usageCount}</span>
                          </TableCell>
                          <TableCell
                            className={
                              'text-[1.5rem] hidden lg:table-cell leading-8 text-neutral-900'
                          ...
                          >
                            <span className={`${isActive ? 'text-success' : 'text-failure'}`}>{usageLimit}</span>
                          </TableCell>
                          <TableCell
                            className={
                              'text-[1.5rem] hidden lg:table-cell leading-8 text-neutral-900'
                          ...
                          >
                            <span className={`${isActive ? 'text-success' : 'text-failure'}`}>{expires.toLocaleString()}</span>
                          </TableCell>
                          <TableCell className={'text-[1.5rem] leading-8 text-neutral-900'}>
                            <Popover>
                              <PopoverTrigger>
                              ...
                              >
                                ...
                                >
                                  ...
                                  <div className={'flex flex-col gap-4'}>
                                    {isActive ? (
                                      <button
                                        onClick={() => MarkAsInactive(discountCodeId)}
                                      ...
                                      <button
                                        onClick={() => MarkAsActive(discountCodeId)}
                                        className={`font-normal cursor-pointer group text-[1.5rem] py-4 leading-[20px] text-neutral-700 hover:text-primary-500 flex items-center justify-between w-full`}
                                      >
          ...
          <TabsContent value="active" className={'w-full h-full '}>
            {event.discountCodes.filter(code => code.isActive).length > 0 ? (
              <Table className={'mt-4'}>
                ...
                <TableBody>
                  {event.discountCodes.filter(code => code.isActive).map(
                    ({
                      code,
                      discountCodeId,
                      expiresAt,
...
function DiscountDrawerContent({ event }: { event: Event }) {
  ...
  const DiscountCodeSchema = z.object({
    code: z.string().min(6, { error: t('errors.code.min') }).max(10, { error: t('errors.code.max') }),
    type: z.string(),
    value: z.string().min(1, { error: t('errors.value.min') }).refine(val => Number(val) >= 1, { error: t('errors.value.negative') }),
    expiresIn: z.string().min(1, { error: t('errors.expiresIn.min') }).refine(val => Number(val) >= 1, { error: t('errors.expiresIn.negative') }),
    usageLimit: z.string().min(1, { error: t('errors.quantity.min') }).refine(val => Number(val) >= 1, { error: t('errors.quantity.negative') })
  })
  type TDiscountCodeSchema = z.infer<typeof DiscountCodeSchema>

  const { register, handleSubmit, setValue, formState: { isSubmitting, errors } } = useForm<TDiscountCodeSchema>({
  ...
  async function submitHandler(data: TDiscountCodeSchema) {
    const result = await CreateDiscountCode(event.eventId, session?.user.accessToken ?? '', { ...data, currencyId: session?.activeOrganisation.currencyId }, pathname)
    if (result.status === 'success') {
      toast.success('success')
    }
    if (result.error) {
  ...
  return (
    <DrawerContent className={'my-6 bg-white p-[30px] rounded-[30px] lg:w-[580px]'}>
      <div className={'w-full flex flex-col items-center overflow-y-scroll'}>
        ...
        <DrawerDescription className='sr-only'>Discount Codes</DrawerDescription>
        <div className={'w-full flex flex-col gap-6'}>
          <div>
            ...
            >
              ...
              <RefreshCircle className='cursor-pointer' onClick={() => {
          ...
          <div>
            ...
            >
              ...
              <span className='text-[1.5rem] leading-[20px] text-neutral-600'>{t('days')}</span>
            </div>
            <span className={"text-[1.2rem] px-8 py-2 text-failure"}>
          ...
          <div>
            <Input type='number' {...register('usageLimit')} min={1} defaultValue={1} error={errors.usageLimit && errors.usageLimit.message}>{t('quantity')}</Input>
            {/* <input
              className={
                'bg-neutral-100 w-full rounded-[5rem] p-8 text-[1.5rem] leading-[20px] placeholder:text-neutral-600 text-deep-200 outline-none border disabled:text-neutral-600 disabled:cursor-not-allowed border-transparent focus:border-primary-500'
              }
      ...
      <DrawerFooter>
        <div className={'flex gap-8'}>
          <DrawerClose className={'flex-1 cursor-pointer'}>
            <div className={'w-full border-primary-500 text-primary-500 bg-primary-100 px-[3rem] py-[15px] border-2 rounded-[100px] text-center font-medium text-[1.5rem] h-auto leading-[20px] cursor-pointer transition-all duration-400 flex items-center justify-center'}>{t('close')}</div>
          </DrawerClose>
          <ButtonPrimary onClick={handleSubmit(submitHandler)} className={'flex-1'}>
