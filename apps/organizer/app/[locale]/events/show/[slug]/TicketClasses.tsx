...
import { Drawer, DrawerClose, DrawerContent, DrawerTitle, DrawerTrigger } from '@workspace/ui/components/drawer'
...
export default function TicketClasses({ event }: { event: Event }) {
  ...
  const ticketTypeSchema = z.object({
    ticketTypes: z.array(z.object({
      ...
      ticketTypeDescription: z.string().min(20, q('errors.ticketClass.description')).max(100),
      ticketTypePrice: z.string().min(1, q('errors.ticketClass.price')),
      ticketTypeQuantity: z.string().min(1, q('errors.ticketClass.quantity.empty')).refine((val) => /^[1-9]\d*$/.test(val), {
  ...
  const { register, control, formState: { errors }, setValue, handleSubmit } = useForm<TTicketTypeSchema>({
    resolver: zodResolver(ticketTypeSchema)
  })

  const [ticketClasses, setTicketClasses] = useState<EventTicketType[]>(event.eventTicketTypes)
  ...
  const getAvailableClassOptions = (index: number) => {
    ...
    return classOptions.filter((option) => !selectedNames.includes(option.value))
  }

  async function submitHandler(data: TTicketTypeSchema) {
    const result = await UpdateTicketTypes(session?.activeOrganisation.organisationId ?? '', event.eventId, session?.user.accessToken ?? '', { ...data, currencyId: session?.activeOrganisation.currencyId }, pathname)
      ...
      <DrawerContent className='lg:w-full h-dvh bg-white px-4 lg:px-28 flex flex-col gap-8'>
        ...
        <div className='flex items-center justify-between pt-8'>
          <div className='flex gap-4 items-center'>
            ...
            <span className=' font-primary font-medium text-[2.6rem] leading-[30px] text-black'>{t('ticket')}</span>
          </div>
          <ButtonPrimary onClick={handleSubmit(submitHandler)} className='hidden lg:flex'>{q('proceed')}</ButtonPrimary>
          <div className='lg:hidden absolute bottom-4 left-0 right-0 w-full flex items-center justify-center px-4'>
            <ButtonPrimary onClick={handleSubmit(submitHandler)} className='w-full' >{q('proceed')}</ButtonPrimary>
          </div>
        </div>
        <div className=" overflow-y-scroll flex flex-col gap-8 overflow-x-hidden">
          ...
          {ticketClasses.length < 3 && <div className='w-full max-w-[540px] mx-auto flex justify-between '>
            <div></div>
            <button
              onClick={() => setTicketClasses(prev => [
                ...prev,
                {
                  ticketTypeName: '',
            ...
            >
              ...
              <span className="text-[1.5rem] leading-8 text-primary-500">{q('add_class')}</span>
            </button>
          </div>}
          <div></div>
