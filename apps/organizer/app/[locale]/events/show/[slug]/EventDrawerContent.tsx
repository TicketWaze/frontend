...
import { Dialog, DialogHeader, DialogTrigger } from "@workspace/ui/components/dialog"
import { DrawerClose, DrawerContent, DrawerDescription, DrawerFooter, DrawerTitle } from "@workspace/ui/components/drawer"
...
export default function EventDrawerContent({ event }: { event: Event }) {
  ...
  return (
    <DrawerContent className={'my-6 p-[30px] rounded-[30px]  lg:w-[580px]'}>
        ...
        <div className={'w-full'}>
          <div className={'w-full gap-[30px] flex flex-col'}>
            ...
            <div className={'w-full flex flex-col gap-[1.5rem] justify-start'}>
              ...
              <p className={'text-[1.5rem] text-neutral-700'}>{event.eventDescription}</p>
            </div>
            <div className={'w-full flex flex-col gap-[1.5rem] justify-start'}>
              <span className={'font-primary text-deep-100 font-medium text-[1.8rem]'}>
                {t('event_details')}
              ...
              <div className={'flex items-center gap-[5px] '}>
                ...
                <span className={'font-normal text-[1.4rem] leading-8 text-deep-200 max-w-[293px]'}>
                  {event.address}
                </span>
              </div>
            </div>
      ...
      <DrawerFooter>
        <DrawerClose className={' cursor-pointer w-full'}>
          <div
            className={'w-full border-primary-500 text-primary-500 bg-primary-100 px-[3rem] py-[15px] border-2  rounded-[100px] text-center font-medium text-[1.5rem] h-auto leading-[20px] cursor-pointer transition-all duration-400 flex items-center justify-center '}>
            {t('close')}
          </div>
        </DrawerClose>
      </DrawerFooter>
